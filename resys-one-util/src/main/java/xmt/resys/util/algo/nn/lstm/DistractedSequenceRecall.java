...
public class DistractedSequenceRecall {
    int tests = 100; // 默认每轮训练100次
    int observation_dimension = 10; // 每个cell的数据维度是10
    ...
    public DistractedSequenceRecall(Random r,
                                    ...
                                    boolean validation_mode) {
        this.r = r;
        this.tests = tests > 0 ? tests : this.tests;
        this.observation_dimension = observation_dimension > 0 ? observation_dimension
                : this.observation_dimension;
    ...
    public double EvaluateFitnessSupervised(IAgentSupervised agent) throws Exception {
        ...
        for (Interaction inter : interactions) {
            ...
            if (inter.target_output == null)
                ...
                if (HBCollectionUtil.argmax(actual_output) == HBCollectionUtil.argmax(inter.target_output))
                    fit++;
                max_fit++;
            }
        }
