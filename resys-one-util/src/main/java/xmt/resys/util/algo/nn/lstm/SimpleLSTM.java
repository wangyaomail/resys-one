package xmt.resys.util.algo.nn.lstm;

import java.util.Random;

import xmt.resys.util.algo.nn.bean.CellBase;
import xmt.resys.util.algo.nn.bean.CellType;

/**
 * LSTM基础模型
 * @INFO 即便是单层的构造（30个神经元左右），在简单的训练后也较为有效
 */
public class SimpleLSTM implements IAgentSupervised {
    private double init_weight_range = 0.1; // 初始化权重范围，意味着权重
    private int full_input_dimension; // =输入数据维度+所有cell数量+1
    private int output_dimension; // 输出维度
    ...
    private double[][] dSdF;
    private double[][] dSdG;
    private CellType CellBase_type_F = CellType.SIGMOID;
    private CellType CellBase_type_G = CellType.SIGMOID;
    private double SCALE_OUTPUT_DELTA = 1.0;
    public static double learningRate = 0.07;// 0.07

    public SimpleLSTM(Random r,
                      int input_dimension,
        ...
        F = CellBase.byType(CellBase_type_F);
        G = CellBase.byType(CellBase_type_G);
        ...
        for (int i = 0; i < full_input_dimension; i++) {
            for (int j = 0; j < cell_blocks; j++) {
                weightsF[j][i] = (r.nextDouble() * 2 - 1) * init_weight_range; // -0.1 ~ +0.1
